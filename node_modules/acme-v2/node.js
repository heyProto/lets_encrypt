...
ACME.challengeTests = {
  'http-01': function (me, auth) {
    ...
    console.log(`challengeTest url:${url}`)
  ...
  return new Promise(function (resolve, reject) {
    ...
    function pollStatus() {
      ...
      return me._request({ method: 'GET', url: ch.url, json: true }).then(function (resp) {
        console.log(`is status processing? ${resp.body.status}`);
        if ('processing' === resp.body.status) {
          console.log("poll processing");
          if (me.debug) console.debug('poll: again');
          return ACME._wait(1 * 1000).then(pollStatus);
        }

        console.log(`is status pending? ${resp.body.status}`);
        ...
        console.log(`is status valid? ${resp.body.status}`);
    ...
    function respondToChallenge() {
      console.log("from respondToChallenge");
      var jws = me.RSA.signJws(
        options.accountKeypair
      , undefined
      , { nonce: me._nonce, alg: 'RS256', url: ch.url, kid: me._kid }
...
ACME._finalizeOrder = function (me, options, validatedDomains) {
  console.log("from finalizeOrder");
  console.log(`validatedDomains ${validatedDomains}`);
  console.log(`options.domainKeypair ${options.domainKeypair}`);
...
ACME._getCertificate = function (me, options) {
  console.log("from _getCertificate");
  ...
  return ACME._getNonce(me).then(function () {
    ...
    , json: jws
      ...
      return next().then(function () {
        ...
        console.log(`_getCertificate validatedDomains=${validatedDomains}`);
